...
static ret_t paint_line_chart(void* ctx, event_t* e) {
  ...
  for (tmp_p = org_p; tmp_p.x <= org_p.x + x_axis_len; tmp_p.x += x_axis_len / x_div_part) {
    ...
    r = rect_init(tk_roundi(tmp_p.x - (c->font_size * lb_str.size / 2)), tmp_p.y + 6,
                  c->font_size * lb_str.size, c->font_size);
    canvas_draw_text_in_rect(c, lb_str.str, lb_str.size, &r);
    wstr_reset(&lb_str);
    canvas_draw_vline(c, tk_roundi(tmp_p.x - (c->font_size * lb_str.size / 2)), tmp_p.y, 6);
  }
  ...
  for (tmp_p = org_p; tmp_p.y >= org_p.y - y_axis_len; tmp_p.y -= y_axis_len / y_div_part) {
    if (i != 0) {
      ...
      wstr_init(&lb_str, 0);
      wstr_from_int(&lb_str, i);
      r = rect_init(tk_roundi(tmp_p.x - 6 - tk_max(c->font_size * (lb_str.size - 1), 0)),
                    tk_roundi(tmp_p.y - c->font_size / 2), c->font_size * lb_str.size,
                    c->font_size);
      canvas_draw_text_in_rect(c, lb_str.str, lb_str.size, &r);
      wstr_reset(&lb_str);
...
static ret_t init_widget(void* ctx, const void* iter) {
  widget_t* widget = WIDGET(iter);
  widget_t* root = widget_get_window(widget);
  widget_t* win = WIDGET(ctx);
...
ret_t application_init(void) {
  ...
  if (!tk_str_eq(APP_DEFAULT_LANGUAGE, "zh") && !tk_str_eq(APP_DEFAULT_LANGUAGE, "en")) {
    ...
    change_language(tk_str_eq(APP_DEFAULT_LANGUAGE, "zh"));
  }
#else
  change_language(FALSE);
