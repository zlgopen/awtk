...
static ret_t skip_unready_workflow(tk_object_t* obj, const char* args, void* ctx) {
  (void)args;
  /* 新 API：orchestrator 自身委托 foreach_prop 到 cmds（cmds_foreach_prop override），
   * 直接对 obj 遍历即可，无需 object_orchestrator_get_cmds。 */
...
static ret_t undo_triggering_workflow(tk_object_t* obj, const char* args, void* ctx) {
  (void)args;
  (void)ctx;
  /* 新 API：orchestrator 自身委托 foreach_prop 到 cmds */
  return tk_object_foreach_prop(obj, undo_triggering_workflow_on_visit, NULL);
}

/* 自定义工作流：执行 c1 后立即重入 orchestrator 的 UNDO，模拟工作流中途触发回滚。
 * 新 API 下首个参数 obj 即 orchestrator 自身（原 ctx 自指已冗余），
 * orchestrator 自身委托 get_prop 到 cmds，直接对 obj 取 c1 即可，无需 get_cmds。 */
static ret_t undo_mid_batch_workflow(tk_object_t* obj, const char* args, void* ctx) {
  ...
  (void)tk_object_exec(obj, TK_OBJECT_CMD_UNDO, args);  /* 重入 orch 的 UNDO */
  return RET_OK;
}
...
TEST(ObjectOrchestrator, CanExecState) {
  ...
  ASSERT_EQ(tk_object_set_prop_object(orch, "c1", c1), RET_OK);
...
TEST(ObjectOrchestrator, ExecUndoOrder) {
  ...
  ASSERT_EQ(tk_object_set_prop_object(orch, "c1", c1), RET_OK);
  ASSERT_EQ(tk_object_set_prop_object(orch, "c2", c2), RET_OK);
  ASSERT_EQ(tk_object_set_prop_object(orch, "c3", c3), RET_OK);
...
TEST(ObjectOrchestrator, CustomWorkflowSkipsUnreadyCmds) {
  ...
  ASSERT_EQ(tk_object_set_prop_object(orch, "theme", c1), RET_OK);
  ASSERT_EQ(tk_object_set_prop_object(orch, "lang", c2), RET_OK);
  ASSERT_EQ(tk_object_set_prop_object(orch, "network", c3), RET_OK);
 ...
 * 兼证便利性：无需 get_cmds，对 orchestrator 直接增指令即获一致的撤销簿记。 */
TEST(ObjectOrchestrator, DynamicCmdAddedDuringWorkflowIsRecordedForUndo) {
  vector<string> log;
  dynamic_cmd_ctx_t info = {NULL, &log};
  tk_object_t* orch = object_orchestrator_create();
  ASSERT_TRUE(orch != NULL);
  ASSERT_EQ(object_orchestrator_set_workflow(orch, dynamic_cmd_workflow, &info), RET_OK);
  ...
  /* orchestrator 委托 #size 到 cmds -> 已注册 1 条指令 */
  ASSERT_EQ(tk_object_get_prop_uint32(orch, TK_OBJECT_PROP_SIZE, 0), 1u);
  ...
  /* 释放：先 orch（cmds 释放 dyn 引用 -> 1），再 dyn 的创建引用 -> 销毁 */
  TK_OBJECT_UNREF(orch);
...
TEST(ObjectOrchestrator, CopyPropsCmdRecordedForUndo) {
  tk_object_t* orch = object_orchestrator_create();
  tk_object_t* src = object_hash_create();
  tk_object_t* c1 = fake_cmd_create("c1");
  ASSERT_EQ(tk_object_set_prop_object(src, "c1", c1), RET_OK);

  ASSERT_EQ(tk_object_copy_props(orch, src, TRUE), RET_OK);
  ASSERT_EQ(tk_object_get_prop_uint32(orch, TK_OBJECT_PROP_SIZE, 0), 1u);
 ...
 * 注：get_cmds 已从公开 API 移除（orchestrator 自身委托 prop 操作到 cmds），clone-cmds 路径经
 * orchestrator 不可达，故此回归直接在 object_hash 层验证（orchestrator 即其 extra_data 消费者）。
 *
...
TEST(ObjectOrchestrator, ObjectHashCloneExtraDataNotCorrupted) {
  ...
  /* 在源的 extra_data 槽位写入哨兵（模拟 orchestrator 反向指针），取实变量地址避免 int→ptr 转换警告 */
