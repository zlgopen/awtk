...
TEST(ObjectHash, set_prop_float) {
  tk_object_t* obj = object_hash_create();
...
TEST(ObjectHash, path) {
  value_t v;
  tk_object_t* root = object_hash_create();
  tk_object_t* obja = object_hash_create();
  tk_object_t* obja1 = object_hash_create();
  tk_object_t* obja2 = object_hash_create();
  tk_object_t* objb = object_hash_create();
  tk_object_t* objb1 = object_hash_create();
  tk_object_t* objb2 = object_hash_create();
...
TEST(ObjectHash, cmd_events_by_path) {
  string log;
  uint32_t i;
  tk_object_t* root = object_hash_create();
  tk_object_t* obja = object_hash_create();
...
TEST(ObjectHash, sub) {
  tk_object_t* obj = object_hash_create();
  tk_object_t* a = object_hash_create();
  tk_object_t* b = object_hash_create();
...
TEST(ObjectHash, disable_path) {
  tk_object_t* obj = object_hash_create_ex(FALSE);
...
TEST(ObjectHash, copy) {
  tk_object_t* obj1 = object_hash_create_ex(FALSE);
  tk_object_t* obj2 = object_hash_create_ex(FALSE);
...
TEST(ObjectHash, find_props) {
  ...
  ASSERT_EQ(value_int((value_t*)darray_get(&matched, 0)), 1001);
  ASSERT_EQ(value_int((value_t*)darray_get(&matched, 1)), 100);
...
TEST(ObjectHash, keep_type) {
  value_t v;
  tk_object_t* obj = object_hash_create();
  object_hash_t* o = OBJECT_HASH(obj);
  object_hash_set_keep_prop_type(obj, TRUE);
