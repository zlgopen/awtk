...
TEST(SHDLC, data_ok) {
  ...
  ASSERT_EQ(shdlc_write_data(wb, 1, "abc", 3), RET_OK);
...
TEST(SHDLC, data_crc_error) {
  ...
  ASSERT_EQ(shdlc_write_data(wb, 3, "abc", 3), RET_OK);
  ASSERT_EQ(wb->cursor, 8);
  ASSERT_EQ(wb->data[0], SHDLC_FLAG);
  ...
  ASSERT_EQ(wb->data[3], 'b');
  ASSERT_EQ(wb->data[4], 'c');
  ASSERT_EQ(wb->data[5], 245);
  ASSERT_EQ(wb->data[6], 158);
  ASSERT_EQ(wb->data[7], SHDLC_FLAG);
...
TEST(SHDLC, data_error) {
  ...
  ASSERT_EQ(shdlc_write_data(wb, 5, "abc", 3), RET_OK);
  ASSERT_EQ(wb->cursor, 8);
  ASSERT_EQ(wb->data[0], SHDLC_FLAG);
