...
TEST(MutableImage, invalidate_force) {
  canvas_t c;
  uint32_t w = 100;
  uint32_t h = 100;
  uint32_t size = w * h * 4;
  ...
  s_is_redraw = false;
  ASSERT_EQ(widget_on_paint_self(img, &c), RET_OK);
  ASSERT_EQ(s_is_redraw, true);

  s_is_redraw = false;
  ASSERT_EQ(mutable_image_invalidate_force(img), RET_OK);
  ASSERT_EQ(widget_on_paint_self(img, &c), RET_OK);
  ASSERT_EQ(s_is_redraw, true);
