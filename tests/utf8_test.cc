...
TEST(Utf8, convert) {
  ...
  test_utf8_convert(
      "覆盖了ASCII、中文、emoji、代理对、超长码点、缓冲区边界、非法输入、截断、空指针等多种情况，极"
      "大提升了健壮性和覆盖率。请编译并运行测试，确保所有场景都能正确通过",
      L"覆盖了ASCII、中文、emoji、代理对、超长码点、缓冲区边界、非法输入、截断、空指针等多种情况，"
      L"极大提升了健壮性和覆盖率。请编译并运行测试，确保所有场景都能正确通过");
}