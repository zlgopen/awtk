...
static ret_t vgcanvas_asset_manager_log_font_destroy(void* vg, const char* font_name, void* specific) {
  int32_t ptr = tk_pointer_to_int(vg);
  switch (ptr) {
    case TEST_VGCANAS1_PTR :
      s_specific_by_vg1 = tk_pointer_to_int(specific);
...
TEST(Vgcanvas_asset_manager, image) {
  ...
  ASSERT_EQ(vgcanvas_asset_manager_add_vg(vgcanvas_asset_manager(), vg1, vgcanvas_asset_manager_log_bitmap_destroy, vgcanvas_asset_manager_log_font_destroy), RET_OK);
  ASSERT_EQ(vgcanvas_asset_manager_add_vg(vgcanvas_asset_manager(), vg2, vgcanvas_asset_manager_log_bitmap_destroy, vgcanvas_asset_manager_log_font_destroy), RET_OK);
  ASSERT_EQ(vgcanvas_asset_manager_add_vg(vgcanvas_asset_manager(), vg3, vgcanvas_asset_manager_log_bitmap_destroy, vgcanvas_asset_manager_log_font_destroy), RET_OK);

  image_loader_load_image(res, &image);
  image.name = "earth";
  
  ASSERT_EQ(vgcanvas_asset_manager_add_image(vgcanvas_asset_manager(), vg1, &image, tk_pointer_from_int(image_ctx1)), RET_OK);
  ASSERT_EQ(vgcanvas_asset_manager_add_image(vgcanvas_asset_manager(), vg2, &image, tk_pointer_from_int(image_ctx2)), RET_OK);

  ctx = tk_pointer_to_int(vgcanvas_asset_manager_get_image_specific(vgcanvas_asset_manager(), vg1, &image, &ret));
  ASSERT_EQ(ctx, image_ctx1);
  ASSERT_EQ(ret, RET_OK);

  ctx = tk_pointer_to_int(vgcanvas_asset_manager_get_image_specific(vgcanvas_asset_manager(), vg2, &image, &ret));
  ...
  ASSERT_EQ(vgcanvas_asset_manager_add_image(vgcanvas_asset_manager(), vg3, &image, tk_pointer_from_int(image_ctx3)), RET_OK);
...
TEST(Vgcanvas_asset_manager, font) {
  ...
  ASSERT_EQ(vgcanvas_asset_manager_add_font(vgcanvas_asset_manager(), vg1, "default", tk_pointer_from_int(font_ctx1)), RET_OK);
  ASSERT_EQ(vgcanvas_asset_manager_add_font(vgcanvas_asset_manager(), vg2, "default", tk_pointer_from_int(font_ctx2)), RET_OK);

  ctx = tk_pointer_to_int(vgcanvas_asset_manager_get_font_ctx(vgcanvas_asset_manager(), vg1, "default", &ret));
  ASSERT_EQ(ctx, font_ctx1);
  ASSERT_EQ(ret, RET_OK);

  ctx = tk_pointer_to_int(vgcanvas_asset_manager_get_font_ctx(vgcanvas_asset_manager(), vg2, "default", &ret));
  ASSERT_EQ(ctx, font_ctx2);
  ASSERT_EQ(ret, RET_OK);

  ctx = tk_pointer_to_int(vgcanvas_asset_manager_get_font_ctx(vgcanvas_asset_manager(), vg3, "default", &ret));
  ASSERT_EQ(ret, RET_FAIL);
  ...
  ASSERT_EQ(vgcanvas_asset_manager_add_font(vgcanvas_asset_manager(), vg3, "default", tk_pointer_from_int(font_ctx3)), RET_OK);
