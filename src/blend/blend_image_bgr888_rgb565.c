...
ret_t blend_image_bgr888_rgb565(bitmap_t* dst, bitmap_t* src, const rectf_t* dst_r, const rectf_t* src_r, uint8_t a) {
  return_value_if_fail(dst != NULL && src != NULL && src_r != NULL && dst_r != NULL,
                       RET_BAD_PARAMS);
  return_value_if_fail(dst->format == BITMAP_FMT_BGR888 && src->format == BITMAP_FMT_RGB565, RET_BAD_PARAMS);
...
ret_t blend_image_rotate_bgr888_rgb565(bitmap_t* dst, bitmap_t* src, const rectf_t* dst_r, const rectf_t* src_r, uint8_t a, lcd_orientation_t o) {
  return_value_if_fail(dst != NULL && src != NULL && src_r != NULL && dst_r != NULL,
                       RET_BAD_PARAMS);
  return_value_if_fail(dst->format == BITMAP_FMT_BGR888 && src->format == BITMAP_FMT_RGB565, RET_BAD_PARAMS);
