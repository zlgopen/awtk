...
ret_t soft_fill_rect(bitmap_t* fb, rect_t* dst, color_t c) {
  ...
  return_value_if_fail(fb->format == BITMAP_FMT_RGB565 || fb->format == BITMAP_FMT_RGBA, RET_BAD_PARAMS); 
...
ret_t soft_copy_image(bitmap_t* fb, bitmap_t* img, rect_t* src, xy_t dx, xy_t dy) {
  ...
  return_value_if_fail(fb->format == BITMAP_FMT_RGB565 || fb->format == BITMAP_FMT_RGBA || fb->format == BITMAP_FMT_BGRA, RET_BAD_PARAMS); 
...
ret_t soft_blend_image(bitmap_t* fb, bitmap_t* img, rect_t* dst, rect_t* src, uint8_t alpha) {
  return_value_if_fail(fb != NULL && img != NULL && src != NULL && dst != NULL,
                       RET_BAD_PARAMS);
