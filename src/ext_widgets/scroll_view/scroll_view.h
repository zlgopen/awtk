...
typedef struct _scroll_view_t {
  ...
  bool_t use_widget_w;
  /**
   * @property {bool_t} use_virtual_h
   * @annotation ["set_prop","get_prop","readable","persitent","design","scriptable"]
   * 是否使用虚拟高度，默认否。
   */
  bool_t use_virtual_h;
  /**
   * @property {bool_t} use_widget_h
   * @annotation ["set_prop","get_prop","readable","persitent","design","scriptable"]
   * 是否使用滚动视图高度，默认否。
   */
  bool_t use_widget_h;
...
#define SCROLL_VIEW_USE_VIRTUAL_H "use_virtual_h"
#define SCROLL_VIEW_USE_WIDGET_H "use_widget_h"
