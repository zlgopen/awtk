...
static ret_t text_selector_set_prop(widget_t* widget, const char* name, const value_t* v) {
  ...
  if (tk_str_eq(name, WIDGET_PROP_VALUE)) {
    ...
    text_selector_parse_options(widget, value_str(v));
    return RET_OK;
  }
...
static ret_t text_selector_on_pointer_up(text_selector_t* text_selector, pointer_event_t* e) {
  ...
  yoffset_end = (yoffset_end/item_height) * item_height;
...
static const widget_vtable_t s_text_selector_vtable = {.size = sizeof(text_selector_t),
