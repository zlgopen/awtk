...
| <a href="#lcd_mem_special_t_lcd_mem_set_wait_vbi">lcd\_mem\_set\_wait\_vbi</a> | 设置等待VBI事件到来的回调函数。 |
...
#### lcd\_mem\_set\_wait\_vbi 函数
-----------------------

* 函数功能：

> <p id="lcd_mem_special_t_lcd_mem_set_wait_vbi">设置等待VBI事件到来的回调函数。
...
| w | wh\_t | 宽度。 |
| h | wh\_t | 高度。 |
| fmt | bitmap\_format\_t | 离线lcd的格式。一般用 BITMAP\_FMT\_BGR565 或 BITMAP\_FMT\_RGBA8888。 |
| on\_flush | lcd\_flush\_t | 回调函数，用于刷新GUI数据到实际的LCD。 |
