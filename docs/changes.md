...
2020/10/20
  ...
  * 重构了窗口动画的 begin_frame 的方法（感谢智明提供补丁）。
  * 修改 label 的 auto_adust_size 功能的换行效果（感谢智明提供补丁）。

2020/10/19
  * 修复窗口焦点控件获取不正常的问题（感谢智明提供补丁）。
  * 恢复 bitmap_destroy 里的改动（会导致 JS 版本野指针的问题）。
  * 修改 auto_adjust_size 缩小功能和注释问题（感谢智明提供补丁）。
  * 修复 edit 由长字符串变为短字符串导致不显示的问题（感谢智明提供补丁）。
  * 修复 text_selector 控件使用翻译和 tr_text 导致重置显示顺序问题（感谢智明提供补丁）。
