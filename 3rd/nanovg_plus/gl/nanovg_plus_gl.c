...
static void nvgp_gl_flush_fill_by_image(nvgp_gl_context_t* gl, nvgp_gl_call_t* call_base) {
  int32_t i;
  nvgp_gl_shader_t* shader = &gl->shader_list[call_base->shader_type];
  nvgp_gl_call_fill_t* call = (nvgp_gl_call_fill_t*)call_base;
  int32_t is_same_shader_prog = nvgp_gl_is_same_shader_prog(gl, shader);
...
static void nvgp_gl_flush(nvgp_gl_context_t* gl) {
  ...
  for (i = 0; i < gl->calls.size; i++) {
    ...
    case NVGP_GL_CALL_CONVEX_FILL:
      ...
      break;
    case NVGP_GL_CALL_FILL_IMAGE :
      nvgp_gl_flush_fill_by_image(gl, call_base);
      break;
...
static nvgp_bool_t nvgp_gl_convert_paint(nvgp_gl_context_t* gl, nvgp_gl_frag_uniforms_t* frag, nvgp_paint_t* paint, nvgp_scissor_t* scissor, float width, float fringe, float strokeThr) {
  ...
  if (scissor->extent[0] < -0.5f || scissor->extent[1] < -0.5f) {
    ...
    float scale_x = 0.0f;
    float scale_y = 0.0f;
    scale_x = nvgp_sqrtf(scissor->matrix.mat.scale_x * scissor->matrix.mat.scale_x + scissor->matrix.mat.skew_x * scissor->matrix.mat.skew_x);
    scale_y = nvgp_sqrtf(scissor->matrix.mat.scale_y * scissor->matrix.mat.scale_y + scissor->matrix.mat.skew_y * scissor->matrix.mat.skew_y);
...
static nvgp_bool_t nvgp_gl_render_fill_by_image(nvgp_gl_context_t* gl, nvgp_paint_t* paint, nvgp_scissor_t* scissor, float fringe, const float* bounds, const nvgp_darray_t* paths, nvgp_fill_mode_t fill_mode) {
  ...
  if (paint->image != 0 && fill_mode != NVGP_FILLMODE_All) {
    ...
    call->base.call_type = NVGP_GL_CALL_FILL_IMAGE;
    call->base.shader_type = NVGP_GL_SHADER_FILLIMG_RGBA;
