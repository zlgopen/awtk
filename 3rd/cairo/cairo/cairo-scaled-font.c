...
_cairo_scaled_font_map_lock (void)
{
	...
	cairo_scaled_font_map = cr_malloc (sizeof (cairo_scaled_font_map_t));
...
_cairo_scaled_font_register_placeholder_and_unlock_font_map (cairo_scaled_font_t *scaled_font)
{
    ...
    placeholder_scaled_font = cr_malloc (sizeof (cairo_scaled_font_t));
  ...
  FREE_PLACEHOLDER:
    cr_free (placeholder_scaled_font);
...
_cairo_scaled_font_create_in_error (cairo_status_t status)
{
	...
	scaled_font = cr_malloc (sizeof (cairo_scaled_font_t));
	if (unlikely (scaled_font == NULL)) {
...
_cairo_scaled_font_reset_static_data (void)
{
    ...
    for (status = CAIRO_STATUS_SUCCESS;
	 ...
	 status++)
    {
	cr_free (_cairo_scaled_font_nil_objects[status]);
				   ...
				   cairo_scaled_glyph_t **scaled_glyph)
{
    ...
    page = cr_malloc (sizeof (cairo_scaled_glyph_page_t));
 ...
 * with the scaled_font so no explicit cr_free is required.
