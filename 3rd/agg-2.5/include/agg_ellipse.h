...
namespace agg
{
    ...
    public:
        ...
        ellipse(float_t x, float_t y, float_t rx, float_t ry, 
        ...
        void init(float_t x, float_t y, float_t rx, float_t ry, 
                  unsigned num_steps=0, bool cw=false);

        void approximation_scale(float_t scale);
    ...
    //------------------------------------------------------------------------
    inline void ellipse::init(float_t x, float_t y, float_t rx, float_t ry, 
    ...
    //------------------------------------------------------------------------
    inline void ellipse::approximation_scale(float_t scale)
    {   
        m_scale = scale;
        calc_num_steps();
    }
    ...
    inline void ellipse::calc_num_steps()
    {
        ...
        float_t da = acos(ra / (ra + 0.125 / m_scale)) * 2;
    ...
    //------------------------------------------------------------------------
    inline unsigned ellipse::vertex(float_t* x, float_t* y)
    {
        if(m_step == m_num) 
        {
            ++m_step;
        ...
        float_t angle = float_t(m_step) / float_t(m_num) * 2.0 * pi;
