...
namespace agg
{
    ...
    struct matrix_pivot
    {
        static int pivot(float_t m[Rows][Cols], unsigned row)
        {
            int k = int(row);
            float_t max_val, tmp;
    ...
    struct simul_eq
    {
        static bool solve(const float_t left[Size][Size], 
                          const float_t right[Size][RightCols],
                          float_t result[Size][RightCols])
        {
            unsigned i, j, k;
            float_t a1;

            float_t tmp[Size][Size + RightCols];
