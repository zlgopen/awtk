...
template <typename T>
inline pod_vector<T>::pod_vector(count_t initial_size) : _begin(0), _end(0), _limit(0) {
  resize(initial_size);
}

template <typename T>
inline pod_vector<T>::pod_vector(const pod_vector& other)
    : _begin(new T[other.capacity()]),
      _end(_begin + other.size()),
      _limit(_begin + other.capacity()) {
  const_iterator i = other.begin();
  iterator j = begin();

  while (i != other.end()) *j++ = *i++;
}
...
inline void pod_vector<T>::resize(count_t size_) {
  if (size_ > capacity()) grow(size_ - capacity());
  _end = _begin + size_;
}
...
inline void pod_vector<T>::assign(count_t size_, const T& value) {
  ...
  for (T* i = _begin; size_; --size_, ++i) *i = value;
}
...
inline void pod_vector<T>::grow(count_t by) {
  ...
  for (iterator i = _begin; i != _end;) *p++ = *i++;
  delete[] _begin;
  _begin = buffer;
  _end = _begin + size;
