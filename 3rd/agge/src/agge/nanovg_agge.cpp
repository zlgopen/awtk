...
#include <agge/nanovg_text_blender.h>
...
static void aggenvg__renderViewport(void* uptr, int width, int height, float devicePixelRatio) {
  NVG_NOTUSED(uptr);
  NVG_NOTUSED(width);
  NVG_NOTUSED(height);
  ...
  if (paint->image > 0) {
    ...
    switch (tex->type) {
      ...
      case NVG_TEXTURE_RGB565: {
        ...
        rgb565_bitmap_t src(tex->width, tex->height, (uint8_t*)(tex->data));
        agge::nanovg_image_blender<PixelT, rgb565_bitmap_t> color(&src, (float*)invxform);
        ren(surface, 0, ras, color, agge::winding<>());
        break;
      }
...
void renderTriangles(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation,
  ...
  for (int i = 0; i < nverts; i += 6) {
    const NVGvertex* v = verts + i;
    ras.move_to(v[0].x, v[0].y);
    ras.line_to(v[2].x, v[2].y);
    ras.line_to(v[1].x, v[1].y);
    ras.line_to(v[4].x, v[4].y);
    ras.close_polygon();
  }
  ...
  if (paint->image > 0) {
    ...
    AGGENVGtexture* tex = aggenvg__findTexture(agge, paint->image);
    nvgTransformInverse(invxform, paint->xform);
    typedef agge::bitmap<agge::pixel8, agge::raw_bitmap> source_bitmap_t;

    source_bitmap_t src(tex->width, tex->height, (uint8_t*)(tex->data));
...
NVGcontext* nvgCreateAGGE(int32_t w, int32_t h, agge_bitmap_format_t format, uint8_t* data) {
  ...
  switch (agge->format) {
    ...
    case AGGE_RGB565: {
      params.renderTriangles = renderTriangles<agge::pixel16_rgb565>;
