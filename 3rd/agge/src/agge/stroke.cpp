...
namespace agge {
stroke::stroke() : _o(_output.end()), _cap(0), _join(0), _state(0) {
}
  ...
  for (; prepare();) {
    ...
    switch (_state & stage_mask) {
      ...
      case outline_forward:
        _join->calc(_output, _width, prev->point, prev->distance, _i->point, _i->distance,
                    next->point);
        _i = next;
        if (_i == last) set_state(end_cap);
        break;
      ...
      case outline_forward_closed:
        _join->calc(_output, _width, prev->point, prev->distance, _i->point, _i->distance,
                    next->point);
        _i = next;
        if (_i == first) set_state(end_poly1);
        break;
      ...
      case outline_backward:
        _join->calc(_output, _width, next->point, _i->distance, _i->point, prev->distance,
                    prev->point);
        _i = prev;
        if (_i == first) set_state(end_poly);
        break;
...
bool stroke::prepare() {
  ...
  if (size() <= (_state & closed ? 2u : 1u)) return false;
